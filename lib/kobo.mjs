 ...
 */
/**
 * Self-test for /api/data?diag=1.
 *
 * Asks BOTH Kobo servers two questions with the configured token: "do you know
 * this user" (/me/) and "do you have this asset". Between them those say which
 * server the token belongs to and whether the asset lives there - which is
 * otherwise guesswork, because an unauthenticated request returns 404 on both
 * servers whether or not the asset exists.
 *
 * It reports the token's LENGTH and SHAPE, never the token. Length is the
 * giveaway when a paste went wrong: a Kobo token is 40 hex characters, so 41
 * means a stray newline came along and 0 means nothing was stored at all.
 */
export async function diagnose(env) {
  const token = (env.KOBO_API_TOKEN || "").trim();
  const uid = (env.KOBO_ASSET_UID || "").trim();
  const raw = env.KOBO_API_TOKEN || "";
  ...
  const probe = async (url) => {
    try {
      ...
      return { status: res.status, statusText: res.statusText,
               body: body ? body.slice(0, 200) : "(empty)" };
    } catch (err) {
      return { status: "fetch failed", detail: String(err.message || err) };
    }
  };

  const hosts = ["https://eu.kobotoolbox.org", "https://kf.kobotoolbox.org"];
  const servers = {};
  for (const h of hosts) {
    servers[h] = {
      whoami: await probe(`${h}/me/?format=json`),
      asset: await probe(`${h}/api/v2/assets/${uid}/?format=json`),
    };
  }

  return {
    configured_server: (env.KOBO_SERVER || "(unset)").trim(),
    asset_uid: uid || "(unset)",
    token_check: {
      stored: raw.length > 0,
      length: raw.length,
      length_after_trim: token.length,
      looks_like_a_kobo_token: /^[0-9a-f]{40}$/.test(token),
      note: raw.length !== token.length
        ? "Stored value has leading or trailing whitespace - it was trimmed before use."
        : token.length && !/^[0-9a-f]{40}$/.test(token)
        ? "Not 40 hex characters. A Kobo API token is 40 hex characters; check what was pasted."
        : "Shape looks right.",
    },
    servers,
    how_to_read_this:
      "whoami 200 means the token belongs to that server. asset 200 means the form "
      + "lives there too. Set KOBO_SERVER to whichever host answers 200 to both.",
  };
}
 ...
 * @param {string} where host name, used only in the "not configured" message
 *                       so the reader is told where to go and set them
 * @param {string} url   the request URL; ?diag=1 runs the self-test instead
 * @returns {Response}
 */
export async function buildResponse(env, where = "your hosting provider", url = "") {
  ...
  // ?diag=1 answers "which server should KOBO_SERVER be, and did the token
  // arrive intact" without disclosing the token itself.
  if (url && new URL(url, "https://x.invalid").searchParams.get("diag")) {
    return json(await diagnose(env), 200, { "cache-control": "no-store" });
  }
