...
| `build_site.py` | Regenerates `public/index.html`, the page the live site serves. |
| `lib/`, `functions/`, `netlify/` | The live site: shared scoring plus one small function per host. |
| `CLOUDFLARE.md` | How the live, always-current dashboard is published. |
...
Everything above builds a dashboard **as at the moment you run it**. The live
version instead updates whenever someone reloads the page, and is published from
this same repository — see **CLOUDFLARE.md**.

It shares all the scoring code, so the two can never disagree: `lib/scoring.mjs`
is a port of `fetch_rdt_results.py` whose labels and answer key are generated by
`gen_js_constants.py`, and `public/index.html` is generated from the same
`rdt_dashboard_template.html`. After changing the template, run
`python build_site.py` and commit the result, or the live site keeps serving the
previous layout.

The GitHub Pages workflow (`.github/workflows/build-dashboard.yml`) is a manual
fallback only. Its schedule is switched off on purpose: this repository is
public, so every run published all the individual submissions to an open URL.

The **Vercel version** (`CNM_RDT_Live_Dashboard.zip`) from the original handoff
is obsolete — it predates the Khmer corrections, the colour work, the added
sections, the submission tracking and the export buttons. Do not deploy it.
